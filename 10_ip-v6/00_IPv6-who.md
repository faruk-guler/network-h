...
İnternet bir anda IPv6'ya geçemez. Bu nedenle geçiş döneminde her iki protokol bir arada kullanılır. Üç temel geçiş mekanizması vardır:

### 1. Dual-Stack (Çift Yığın)

Bir cihazda hem IPv4 hem de IPv6 adresi aynı anda bulunur. Hedefe göre uygun protokol otomatik seçilir. Günümüzdeki modern cihazların büyük çoğunluğu bu yöntemi kullanır.
...
### 2. Tunneling (Tünelleme)

IPv6 paketlerinin, henüz IPv6 desteklemeyen IPv4 altyapısı üzerinden taşınmasını sağlar. IPv6 paketi bir IPv4 paketinin içine kapsüllenir, karşı uçta tekrar açılır.

- **6to4:** Otomatik tünelleme; her IPv4 adresine karşılık bir IPv6 prefix üretir.
- **Teredo:** NAT arkasındaki cihazların IPv6 tüneli kurabilmesini sağlar (UDP üzerinden).
- **ISATAP:** Kurum içi (intranet) IPv4 ağlarında IPv6 tüneli oluşturur.

### 3. NAT64 / DNS64

IPv6-only (yalnızca IPv6 adresi olan) cihazların, IPv4-only sunuculara erişmesini sağlayan çeviri mekanizmasıdır.

- **DNS64:** IPv4 adresini sentezlenmiş bir IPv6 adresine çevirir.
- **NAT64:** IPv6 paketini alır, IPv4 paketine dönüştürür ve hedefe iletir.

> 💡 **Güncel Eğilim:** ISP'ler yeni aboneliklerde giderek daha fazla **IPv6-only + NAT64** modelini benimsemektedir.
...
| Geçiş | Dual-Stack, Tunneling (6to4, Teredo) ve NAT64/DNS64 |
